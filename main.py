...
def onclick(event):
    ...
    plt.plot(event.xdata, event.ydata,marker='D' , markersize=3,color='r') 
    dp.append((event.xdata, event.ydata))
    dpx.append(event.xdata)
    dpy.append(event.ydata)
    # function to show the plot 
    ax.annotate(i, (event.xdata, event.ydata))
    #plt.plot(dpx,dpy)
    if(click_number==0):
    	click_number=1
    else:
    	plt.arrow(dpx[i-1],dpy[i-1],dpx[i]-dpx[i-1],dpy[i]-dpy[i-1],width=0.05)
    i+=1
    plt.show()
...
def mysecondplot(alist):
    ...
    def onclick1(event):
        ...
        #plt.plot(event.xdata, event.ydata, ',')
        while(i1<len(alist)):
            plt.plot(dpx[alist[i1]], dpy[alist[i1]],marker='D' , markersize=3,color='r')  
            ax.annotate(alist[i1], (dpx[alist[i1]], dpy[alist[i1]]))
            if(i1==0):
                clickn=1
            else:
                plt.arrow(dpx[alist[i1-1]],dpy[alist[i1-1]],dpx[alist[i1]]-dpx[alist[i1-1]],dpy[alist[i1]]-dpy[alist[i1-1]],width=0.05)
            i1+=1
            print (i1)
        plt.show()
    i1=0
    clickn=0
    n=len(alist)
    axcut = fig1.add_axes([0.0, 0.0, 0.1, 0.075])
